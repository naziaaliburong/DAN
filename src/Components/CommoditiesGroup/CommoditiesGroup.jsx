...
import { getCommodity } from '../../API/commodityAPI'
import fruit from "../../Assets/Images/fruits.jpg"

const CommoditiesGroup = ({ selectedCommodity, handleSelect, handleNext }) => {
    const [commoditiess, setCommoditiess] = useState(commodities);

    useEffect(()=>{
        const fetchData = async ()=>{
            const response = await getCommodity();
            console.log(response);
            setCommoditiess(response.map(item => ({
                id: item.id,
                title: item.name,
                image: item.image===null&&fruit
            })));
        };
        fetchData();
    },[]);
